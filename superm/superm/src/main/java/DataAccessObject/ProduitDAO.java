...
public class ProduitDAO {
    ...
    public int create(Produit produit) throws SQLException {
        String sql = "INSERT INTO produits (nom, description, prix, image, quantite) VALUES (?, ?, ?, ?, ?)";
        try (PreparedStatement stmt = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            stmt.setString(1, produit.getNom());
            stmt.setString(2, produit.getDescription());
            stmt.setDouble(3, produit.getPrix());
            stmt.setString(4, produit.getImage());
            stmt.setInt(5, produit.getQuantite()); // Add quantity
    ...
    public List<Produit> getAll() throws SQLException {
        ...
        String sql = "SELECT id, nom, description, prix, image, quantite FROM produits";
        try (PreparedStatement stmt = connection.prepareStatement(sql);
    ...
    public Produit getById(int id) throws SQLException {
        String sql = "SELECT id, nom, description, prix, image, quantite FROM produits WHERE id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setInt(1, id);
    ...
    public void update(Produit produit) throws SQLException {
        String sql = "UPDATE produits SET nom = ?, description = ?, prix = ?, image = ?, quantite = ? WHERE id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setString(1, produit.getNom());
            stmt.setString(2, produit.getDescription());
            stmt.setDouble(3, produit.getPrix());
            stmt.setString(4, produit.getImage());
            stmt.setInt(5, produit.getQuantite()); // Update quantity
    ...
    public void updateQuantite(int id, int quantite) throws SQLException {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            throw new SQLException("Error updating quantity for product ID " + id + ": " + e.getMessage());
        }
    }
    ...
    private Produit mapRowToProduit(ResultSet rs) throws SQLException {
        ...
        produit.setQuantite(rs.getInt("quantite")); // Map quantity
        return produit;
    }
}
